...
#include "../../../../../../../../../../lib/opusfile/opusfile.h"
...
Kernel::handlePacket(const Babel::Common::Network::Packet &packet)
{
    ...
    this->_network->SendToClient(packet);
    return true;
}

std::list<Babel::Audio::IEncode::t_packet>
Kernel::encode(SAMPLE *data, unsigned int size) const
{
...
SAMPLE
*Kernel::decode(const std::list<Babel::Audio::IEncode::t_packet> &packet, unsigned int *size) const
{
    OpusDecoder				*dec;
    int					error;
    SAMPLE					*r;
    ...
    *size = 0;
    ...
    while (it != packet.end())
    {
        Babel::Audio::IEncode::t_packet	pack = (*it);
        s = opus_decode_float(
                    dec,
                    pack.data,
                    pack.size,
